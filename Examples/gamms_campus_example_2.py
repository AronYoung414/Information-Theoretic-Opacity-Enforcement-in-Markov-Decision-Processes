...
node_coords = {
    0: (-250, 50), 1: (50, 500), 2: (200, 200), 3: (200, 800),
    ...
    15: (950, 300), 21: (950, 500), 28: (950, 700), 23: (750, 900),
...
ctx.visual.set_graph_visual(width=2400, height=1400)

sensor_coverage = {
    0: [1, 2, 3, 4], 1: [7, 8, 9], 2: [11, 12, 13, 14],
    3: [16, 17, 18, 19], 4: [20, 21, 22], 5: [25, 26, 27, 28],
...
class ZoneMonitorSensor(gamms.typing.ISensor):
    ...
    def sense(self, node_id: int):
        cam_num = int(self._sensor_id.split('_')[1]) + 1
...
def monitor_active_sensing_strategy(state):
    ...
    query_str = f"Cam {current_action_idx + 1}" if current_action_idx < 9 else "No Query"
    print(f"Ego: {ego_pos:<2} | Traffic 1: {t1_pos:<2} | Traffic 2: {t2_pos:<2} | Query: {query_str:<6} | Observation: {current_obs_tuple}")
...
# # ctx.visual.set_agent_visual(name='monitor', color=(0, 0, 0), size={'radius': 5})
...
#         if n1 and n2:
#             # Positional matching: (x1, y1, x2, y2, color, width, is_aa, perform_culling_test)
#             draw_ctx.visual.render_line(n1.x, n1.y, n2.x, n2.y, (80, 80, 80), 6, True, False)
#
#     for nid in node_coords.keys():
#         n = draw_ctx.graph.graph.get_node(nid)
#         if n:
#             # Positional matching: (x, y, radius, color, width, perform_culling_test)
#             draw_ctx.visual.render_circle(n.x, n.y, 30, (30, 30, 30), 0, False)
#
#             try:
#                 # Positional/Kwargs matching: (text, x, y, color, perform_culling_test, font_size)
#                 draw_ctx.visual.render_text(str(nid), n.x + 35, n.y - 25, (0, 0, 0), False, 18)
#             except Exception:
#                 pass
#
#
...
#         if node:
#             # Ring for coverage
#             draw_ctx.visual.render_circle(node.x, node.y, 50, (255, 255, 0), 8, False)
#             if is_detected:
#                 # Solid fill for detection
#                 draw_ctx.visual.render_circle(node.x, node.y, 30, (0, 255, 0), 0, False)
#
#
# ctx.visual.add_artist("query_viz", gamms.visual.Artist(ctx, drawer=draw_active_sensor, layer=50))

# --- 6. INITIALIZE AGENTS (100% NATIVE GAMMS) ---
ctx.agent.create_agent(name='monitor', start_node_id=0)
ctx.agent.get_agent('monitor').register_strategy(monitor_active_sensing_strategy)

ctx.agent.create_agent(name='ego', start_node_id=1)
# Injecting shape='triangle' and layer=40 directly into the native wrapper!
ctx.visual.set_agent_visual(name='ego', color=(0, 0, 255), size=55.0)  # type: ignore
ctx.agent.get_agent('ego').register_strategy(ego_policy)

ctx.agent.create_agent(name='traffic_1', start_node_id=3)
ctx.visual.set_agent_visual(name='traffic_1', color=(255, 0, 0), size=45.0)  # type: ignore
ctx.agent.get_agent('traffic_1').register_strategy(traffic_1_policy)

ctx.agent.create_agent(name='traffic_2', start_node_id=29)
ctx.visual.set_agent_visual(name='traffic_2', color=(255, 0, 0), size=45.0)  # type: ignore
ctx.agent.get_agent('traffic_2').register_strategy(traffic_2_policy)

for action_id, target_nodes in sensor_coverage.items():
    s_name = f"cam_{action_id}"
    sensor = ZoneMonitorSensor(ctx, s_name, target_nodes, 'ego', ['traffic_1', 'traffic_2'], false_negative_rate=0.0)
    ctx.sensor.add_sensor(sensor)
    ctx.agent.get_agent('monitor').register_sensor(s_name, sensor)


# --- 7. VISUALIZATION ARTISTS (CLEANED UP) ---
def draw_enhanced_graph(draw_ctx, data):
    for (u, v) in edges:
        n1 = draw_ctx.graph.graph.get_node(u)
        n2 = draw_ctx.graph.graph.get_node(v)
...
def draw_active_sensor(draw_ctx, data):
    ...
    for node_id in target_nodes:
        ...
        if node:
            ...
            # We only load the floor (graph) and the ceiling (sensors). GAMMS natively handles the agents now!
...
# --- 8. MAIN LOOP ---
print("Simulation initialized. Waiting 5 seconds for screen recording setup...")
time.sleep(5.0)
