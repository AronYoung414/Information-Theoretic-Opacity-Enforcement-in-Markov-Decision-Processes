...
def campus_example_predictive_safety(iter_num=1000, batch_size=100, V=100, T=3, lr=0.005, k_steps=1, alpha=0,
                                     num_test_episodes=1000, fp_noise=0):
    ...
    for s in states:
        ...
        # elif e in buffer_nodes:   # Setting buffers nodes as goal to see if changing the label changes
        #     labels[s] = {'goal'}
        else:
    ...
    ego_start = {1, 44}
    zone_1_start = {3, 6, 5, 7}
    for ego_start, c_start in itertools.product(ego_start, zone_1_start):
    ...
    cam10_nodes = [19]  #TODO: Added this to see if it makes things better.
    ...
    covered_nodes = set(cam1_nodes + cam2_nodes + cam3_nodes + cam4_nodes + cam5_nodes +
                        cam6_nodes + cam7_nodes + cam8_nodes + cam9_nodes + cam10_nodes)
    ...
    setC10_E, setC10_T = build_cov(cam10_nodes)
    ...
    sensing_action = {
        0: {'NO_E', 'NO_T'},  # Action 9 is now NO sensing
        1: {'C1_E', 'C1_T'},
        2: {'C2_E', 'C2_T'},
        3: {'C3_E', 'C3_T'},
        4: {'C4_E', 'C4_T'},
    ...
    no_sense_act = 0  # Action 0 is the NO sensing action
    ...
    for i, (set_E, set_T) in enumerate([
        ...
        (setC7_E, setC7_T), (setC8_E, setC8_T), (setC9_E, setC9_T), (setC10_E, setC10_T)
    ], start=1):
