...
def running_example_predictive_safety(iter_num=1000, batch_size=100, V=100, T=3, lr=0.005, k_steps=1, alpha=0,
    ...
    logger.add("logs_for_examples/log_file_for_running_example_predictive_safety.log")
    ...
    prob = {
        'a': np.array([[0, 1, 0, 0, 0, 0], [0, 0, 0.8, 0, 0, 0.2], [0, 0, 0, 0.8, 0, 0.2],
    ...
    # SAVE MODEL --- When needed to save the learnt LSTM model.
    # model_filename = "policy_for_gamms_running_example_1.pth"
    ...
    # avg_brier_score, acc = lstm_solver.evaluate_brier_score_agent(episodes=num_test_episodes, horizon=T, k_step=k_steps,
    #                                                               uniform_random=False)
    #
    # # Evaluate the Brier score for the uniform random policy.
    # avg_brier_score_uni_rdn, acc_uni_rdn = lstm_solver.evaluate_brier_score_agent(episodes=num_test_episodes, horizon=T,
    #                                                                               k_step=k_steps, uniform_random=True)
    #
    # # Evaluate Brier score and accuracy for the Oracle baseline.
    # avg_brier_score_oracle, acc_oracle = lstm_solver.evaluate_oracle_baseline(episodes=num_test_episodes, horizon=T, k_step=k_steps)

    # Evaluate the Brier score for the trained policy.
    avg_brier_score, ci_brier, acc, ci_acc = lstm_solver.evaluate_brier_score_agent(episodes=num_test_episodes,
                                                                                    horizon=T, k_step=k_steps,
                                                                                    uniform_random=False)

    # Evaluate the Brier score for the uniform random policy.
    avg_brier_score_uni_rdn, ci_brier_uni_rdn, acc_uni_rdn, ci_acc_uni_rdn = lstm_solver.evaluate_brier_score_agent(
        episodes=num_test_episodes, horizon=T,
        k_step=k_steps, uniform_random=True)
    ...
    print(f"Average Brier Score for the trained policy: {avg_brier_score:.6f} ± {ci_brier:.6f} ")
    print(f"Accuracy for the trained policy based on Brier Score evaluation: {acc:.6f}% ± {ci_acc:.6f}%")

    print(f"Average Brier Score for the uniform random policy: {avg_brier_score_uni_rdn:.6f} ± {ci_brier_uni_rdn:.6f}")
    print(
        f"Accuracy for the uniform random policy based on Brier Score evaluation: {acc_uni_rdn:.6f}% ± {ci_acc_uni_rdn:.6f}%")

    print(f"Average Brier Score for the Oracle baseline: {avg_brier_score_oracle:.6f} ± {ci_brier_oracle:.6f}")
    print(f"Accuracy for the Oracle baseline based on Brier Score evaluation: {acc_oracle:.6f}% ± {ci_acc_oracle:.6f}%")
